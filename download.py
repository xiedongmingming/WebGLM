...
def parse_args():
    ...
    parser.add_argument(
        '--link', '-l', type=str, required=True, help='Share link of Tsinghua Cloud'
    )
    parser.add_argument(
        '--password', '-p', type=str, default='', help='Password of the share link'
    )
    parser.add_argument(
        '--save', '-s', type=str, default='./', help='Save directory'
    )
    parser.add_argument(
        '--file', '-f', type=str, default=None, help='File name, support regex, if not set, download all files'
    )
...
if __name__ == "__main__":
    ...
    if args.target in ["all", "retriever-training-data"]:
        #
        download(
            "https://cloud.tsinghua.edu.cn/d/fa5e6eb1afac4f08a4c6/",
    ...
    if args.target in ["all", "retriever-pretrained-checkpoint"]:
        #
        download(
            "https://cloud.tsinghua.edu.cn/d/54056861b2f34bbfb3f9/",
